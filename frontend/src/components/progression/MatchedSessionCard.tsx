...
function DurationDelta({ pct }: { pct: number | null }) {
  ...
  const sign = pct > 0 ? '+' : '−'
  const color =
    abs <= 15 ? 'text-emerald-700 dark:text-emerald-300 border-emerald-500/30 bg-emerald-500/10'
    : abs <= 30 ? 'text-amber-700 dark:text-amber-300 border-amber-500/30 bg-amber-500/10'
    : pct < 0   ? 'text-red-700 dark:text-red-300 border-red-500/30 bg-red-500/10'
