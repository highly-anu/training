          ...
          {rows.map(({ match, workout }) => {
            ...
            return (
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => navigate(`/import/${encodeURIComponent(workout.id)}`, { state: { workout } })}
                onKeyDown={(e) => e.key === 'Enter' && navigate(`/import/${encodeURIComponent(workout.id)}`, { state: { workout } })}
                className="flex items-center gap-3 rounded-lg border border-border/30 bg-card/40 px-3 py-2.5 cursor-pointer hover:bg-card/60 transition-colors"
              >
                <FileText className="size-4 shrink-0 text-muted-foreground" />
                <div className="flex-1 min-w-0">
