...
export interface ReadinessResult {
  ...
  flags: ReadinessFlag[]
  components: { rhr: number; hrv: number; fatigue: number }
}
...
// ── Component 1: Resting HR (0–40 pts) ────────────────────────────────────────
...
function scoreRHR(bioLogs: DailyBioLog[], flags: ReadinessFlag[]): number {
  ...
  if (delta <= 3) return Math.round(40 - (delta / 3) * 10)
  if (delta <= 7) return Math.round(30 - ((delta - 3) / 4) * 15)
  return Math.max(0, Math.round(15 - (delta - 7) * 2))
}

// ── Component 2: HRV (0–40 pts) ───────────────────────────────────────────────
...
// ── Component 3: Session fatigue (0–20 pts) ───────────────────────────────────
...
function scoreFatigue(
  ...
  if (logs.length === 0) return 12 // neutral
...
export function computeReadiness(
  ...
  const score = Math.min(100, Math.max(0, rhr + hrv + fatigue))
  const status = score >= 70 ? 'green' : score >= 45 ? 'yellow' : 'red'

  return { score, status, flags, components: { rhr, hrv, fatigue } }
}
